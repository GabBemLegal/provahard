...
# Frame global usado para atualizar a lista após edição
frame_global_lista = None
...
def abrir_editor_envio(envio):
    ...
    for i, camp in enumerate(campos):
        ...
        valor = envio[camp] if envio[camp] is not None else ""
    ...
    # --- função que salva, fecha e recarrega tabela ---
    def salvar_e_fechar():
        ...
        messagebox.showinfo("OK", "Alterações salvas com sucesso!")

        # Fecha a janela e recarrega lista
        janela.destroy()
        if frame_global_lista:
            carregar_envios(frame_global_lista)

    ctk.CTkButton(janela, text="Salvar Alterações", command=salvar_e_fechar).pack(pady=20)


def abrir_tela_gerenciar_envios():
    global frame_global_lista
    ...
    frame_global_lista = ctk.CTkScrollableFrame(janela, width=650, height=550)
    frame_global_lista.pack(pady=20)
